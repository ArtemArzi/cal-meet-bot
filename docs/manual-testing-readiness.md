...
3. `/people` — проверка: доступна только менеджерам в ЛС. Для теста убедитесь, что ваш `telegram_user_id` помечен как менеджер в БД (таблица `users`, колонка `is_manager=1`).

### Проверка календаря
4. Создайте событие в Google Calendar.
5. Проверьте логи: `POST /calendar/webhook` должен вернуть 200.
6. Убедитесь, что встреча появилась в БД и участникам ушли уведомления.

### Путь участника
7. Участник нажимает «Подтвердить» в ЛС. Проверьте смену статуса и уведомления.
8. Участник нажимает «Отклонить». Если требуется решение организатора, проверьте переход в статус `needs_initiator_decision`.

### Путь организатора
9. Организатор получает кнопки для принятия решения в ЛС.
10. Выберите действие и проверьте финальный статус (`confirmed` или `cancelled`).
11. Проверьте, что событие в календаре обновилось.

### Обновление события
12. Перенесите время события в Google Calendar. Цикл подтверждения должен начаться заново.
13. Удалите событие в календаре. Встреча должна перейти в `cancelled`.
